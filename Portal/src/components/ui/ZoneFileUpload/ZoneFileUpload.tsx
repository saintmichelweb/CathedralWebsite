  ...
  return (
      ...
      onDrop={e => {
        ...
        if (fileList) {
          ...
          if (fileList.length > 3) {
            setFileError(true)
            setFileErrorMessage('Sorry, only 3 file allowed to be attached at once...')
            return
    ...
    >
      <Stack content='center' align='center' spacing='3'>
        ...
        <VisuallyHiddenInput
          ...
          onChange={e => {
            ...
            if (fileList) {
              ...
              if (fileList.length > 3) {
                setFileError(true)
                setFileErrorMessage('Sorry, only 3 file allowed to attach at once...')
                return
              } else if (!files.some(file => allowedTypes.includes(file.type))) {
                setFileError(true)
                setFileErrorMessage('Sorry, only pdf and image file format allowed...')
        ...
        >
          {uploadProgress === 0 ? 'Browse Files' : 'Change file'}
        </CustomButton>
      </Stack>
    </Flex>
  )
